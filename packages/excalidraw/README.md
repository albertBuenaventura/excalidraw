...
```bash
npm install react react-dom @albertBuenaventura/excalidraw
# or
yarn add react react-dom @albertBuenaventura/excalidraw
```

> **Note**: If you want to try unreleased changes, use `@albertBuenaventura/excalidraw@next`.
...
- If imports or entrypoints are unclear, inspect `node_modules/@albertBuenaventura/excalidraw/package.json`. The installed package exports are the source of truth.
...
## Migrating to `@albertBuenaventura/excalidraw@0.18.x`

Version `0.18.x` removes the old `types/`-prefixed deep import paths. If you were importing types from `@albertBuenaventura/excalidraw/types/...`, switch to the new type-only subpaths below.

| Old path | New path |
| --- | --- |
| `@albertBuenaventura/excalidraw/types/data/transform.js` | `@albertBuenaventura/excalidraw/element/transform` |
| `@albertBuenaventura/excalidraw/types/data/types.js` | `@albertBuenaventura/excalidraw/data/types` |
| `@albertBuenaventura/excalidraw/types/element/types.js` | `@albertBuenaventura/excalidraw/element/types` |
| `@albertBuenaventura/excalidraw/types/utility-types.js` | `@albertBuenaventura/excalidraw/common/utility-types` |
| `@albertBuenaventura/excalidraw/types/types.js` | `@albertBuenaventura/excalidraw/types` |

Drop the `.js` extension. The new package `exports` map resolves these paths without it.

These deep subpaths are for `import type` only. Runtime imports should come from the package root, plus `@albertBuenaventura/excalidraw/index.css` for styles.

For example:

```ts
import { exportToSvg } from "@albertBuenaventura/excalidraw";
```

## Self-hosting fonts

By default, Excalidraw downloads the fonts it needs from the [CDN](https://esm.run/@albertBuenaventura/excalidraw/dist/prod).

For self-hosting, copy the contents of `node_modules/@albertBuenaventura/excalidraw/dist/prod/fonts` into the path where your app serves static assets, for example `public/`. Then set `window.EXCALIDRAW_ASSET_PATH` to that same path:
...
Read the [integration docs](https://docs.excalidraw.com/docs/@albertBuenaventura/excalidraw/integration).

## API

Read the [API docs](https://docs.excalidraw.com/docs/@albertBuenaventura/excalidraw/api).

## Contributing

Read the [contributing docs](https://docs.excalidraw.com/docs/@albertBuenaventura/excalidraw/contributing).
